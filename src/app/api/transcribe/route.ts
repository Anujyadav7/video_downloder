 ...
 * Uses internal Service Binding to bypass 1003 and includes Hinglish logic.
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!body.url) return NextResponse.json({ status: "error", error: "Media URL is required" }, { status: 400 });
    ...
    // --- 1. MEDIA EXTRACTION (Service Binding) ---
    try {
        ...
        if (!mediaUrl && data.status === "error") {
            throw new Error(`Engine Error: ${data.text || data.error || 'COBALT_FAIL'}`);
        }
    } catch (e: any) {
        return NextResponse.json({ status: "error", error: "Extraction Failed", details: e.message }, { status: 502 });
    }

    if (!mediaUrl) return NextResponse.json({ status: "error", error: "No direct media link found." }, { status: 422 });
    ...
    return NextResponse.json({ status: "error", error: "Transcription processor failure", message: error.message }, { status: 500 });
  }
}
